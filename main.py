        ...
        if USE_SINGLE_KEY_INPUT and getch_fn:
            ...
            while True:
                ...
                if user_key and user_key.isdigit() and 1 <= int(user_key) <= len(options_to_display):
                    ...
                    print(user_key) 
                    break
                elif user_key and user_key.lower() in ['q', 'й']: # Выход по 'q' или 'й'
            ...
            while True:
                try:
                    user_input_str = input("Ваш ответ (введите номер и нажмите Enter): ")
                    ...
                    if 0 <= user_choice_index < len(options_to_display):
                        ...
                        print(f"Ошибка: введите число от 1 до {len(options_to_display)}.")
                except ValueError:
...
if __name__ == "__main__":
    while True:
        ...
        # Для главного меню используем стандартный input, так как здесь одна клавиша не так критична
        # и это упрощает код (не нужно обрабатывать каждый символ отдельно для меню)
        choice = input("Выберите тест (1-4) и нажмите Enter: ")
        
        if choice == '1':
            run_test(list(exam_grigoriev_data), shuffle_options=SHUFFLE_ANSWER_OPTIONS)
        elif choice == '2':
            run_test(list(exam_sergeev_data), shuffle_options=SHUFFLE_ANSWER_OPTIONS)
        elif choice == '3':
            questions = load_questions_from_json('questions_from_file.json')
            if questions:
                run_test(list(questions), shuffle_options=SHUFFLE_ANSWER_OPTIONS)
        elif choice == '4':
            print("До встречи!")
            break
        else:
